...
print_rule(symbol lhs, NODE* rule_rhs) {
    printf("%s -> ", nonterminaltoString(lhs.nt));
    NODE* temp = rule_rhs;
    while(temp != NULL) {
        if(temp->sym.is_terminal) {
            printf("%s ", tokenToString(temp->sym.t));
        } else {
            printf("%s ", nonterminaltoString(temp->sym.nt));
        }
        temp = temp->next;
    }
    printf("\n");
...
int main(void) {
    ...
    while(buffer[forwardPtr] != EOF) {
        ...
        if(sym.is_terminal==0 && predictive_table[sym.nt][curr.name].is_syn==0 && predictive_table[sym.nt][curr.name].is_error==0){
            ...
            print_rule(top(stack), predictive_table[sym.nt][curr.name].rule_rhs);
            call_token=0;
